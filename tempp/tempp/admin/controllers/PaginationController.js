const PaginationController = {
  Initialize: function () {
    ...
    for (let i = 1; i <= ProductModel.getTotalPage(); i++) {
      ...
      $(".page-product ul li:last-child").insertAdjacentHTML(
        "beforeBegin",
        page
      );
    }
    for (let i = 1; i <= CategoryModel.getTotalPage(); i++) {
      ...
      $(".page-category ul li:last-child").insertAdjacentHTML(
        "beforeBegin",
        page
      );
    }
    for (let i = 1; i <= BillModel.getTotalPage(); i++) {
      ...
      $(".page-bill ul li:last-child").insertAdjacentHTML(
        "beforeBegin",
        page
      );
    }
    ...
    $$(".page-user ul li").forEach((page) => {
      page.onclick =  function () {
        ...
        collection.forEach((document) => {
          result += `
              <tr>
                  <td contenteditable="true" class="username">${
                    document.username
                  }</td>
                  <td class="isAdmin">                
                      <label class="switch">
                          <input type="checkbox" ${
                            document.isAdmin ? "checked" : ""
                          }>
        ...
        collection.forEach((document) => {
          ...
          result += `
            ...
            <div class="wrap-image">
              ...
              <td contenteditable="true" class="priceProduct">${formatNumber(document.price)}</td>
              <td class="rateProduct">${document.rate}</td>
              <td class="action">
  ...
  Update: function (page) {
    if (page == "user") {
      ...
      for (let i = 1; i <= UserModel.getTotalPage(); i++) {
        ...
        $(".page-user ul li:last-child").insertAdjacentHTML(
          "beforeBegin",
          page
        );
      }
