const HandleEvent = {
  ...
  Product : {
      ...
      <tr>
        <td>
          <div class="wrap-image" style="cursor:not-allowed">
            <img class="image-document i-product" src="./images/products/product-1-img-1.jpg" alt="">
          ...
          <td contenteditable="true" class="priceProduct">priceProduct</td>
          <td class="rateProduct">null</td>
          <td class="action">
    ...
    Save : function(item){
      ...
      let product = {
        ...
        rate : row.querySelector(".rateProduct").textContent,
        price : row.querySelector(".priceProduct").textContent.replaceAll(".",""),
      }
  ...
  Revenue : {
    Filter : function(){
      let from = $(".input-from").value != "" ? Date.parse($(".input-from").value) : ""
      let to = $(".input-to").value != "" ? Date.parse($(".input-to").value) : ""
      let category = $("#select-search-category").value;
      let bills = RevenueController.getRevenue()[1];
      ...
      if((from == "" && to != "" ) || (from != "" && to == "")){
        ...
        // if choose category load product by category
        if(from == "" && to == "" && category != "0"){
          products.forEach(item => {
            data.item = item;
          ...
          RevenueView.renderSubtotalToView(data);
        }
        // if nothing => load all
        if(from == "" && to == "" && category == "0"){
          RevenueView.Load();
          ...
          if(from >= to){
            ...
            products.forEach(item => {
              ...
              if(category == "0"){
                let inRange = false;
                bills.forEach(bill => {
                  if(from <= bill.created_at && to >= bill.created_at){
                    bill.products.forEach(product => {
                ...
                if(inRange){
                  data.subtotal += item.price * amountSold
                  data.rowProduct += 
                  `<tr>
                    <td>${data.item.name}</td>
                    <td>${data.item.category}</td>
                    <td>${formatNumber(data.item.price)}</td>
                    <td>${amountSold}</td>
                    <td>${formatNumber(data.item.price * amountSold)}</td>
                  </tr>`
                }
              }else{
                if(item.category == category){
