  ...
  updateNameCategory: function (id, newNameCategory, img, oldNameCategory) {
    if (Validate.isLength(newNameCategory, options.category.name)) {
      ...
      for (const document of collection) {
        if (document.id == id) {
          found = true;
          if (document.name == newNameCategory && document.img == img) {
            return { status: false, message: lang.nothingChangeDocument };
          } else {
            if (document.img != img) {
              document.name = newNameCategory;
              document.img = img;
              this.UpdateAll(collection);
              ProductModel.updateCategoryProduct(oldNameCategory, newNameCategory)
