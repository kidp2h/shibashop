const ProductModel = {
  ...
  getTotalPage_Rate : function(LIMIT){
    return totalPageUser = this.getAll().filter(product => product.rate == 5).length % LIMIT == 0 ? this.getAll().filter(product => product.rate == 5).length / LIMIT :  (this.getAll().filter(product => product.rate == 5).length / LIMIT) + 1
  },

  getDocumentsByPage_Rate : function(page, LIMIT) {
      return this.getAll().filter(product => product.rate == 5)
